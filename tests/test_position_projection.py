...
class PositionProjectionTest(unittest.TestCase):
    ...
    def test_lead_is_clamped_to_track_duration(self) -> None:
    ...
    def test_negative_lead_shifts_published_position_backward(self) -> None:
        projector = PositionProjector(clock=self.clock, lead_us=-400_000)
        projector.update(self.session)
    ...
    def test_negative_lead_stops_at_zero_near_track_start(self) -> None:
        projector = PositionProjector(clock=self.clock, lead_us=-400_000)
        near_start = copy.deepcopy(self.session)
        near_start["playbackState"]["positionMs"] = 100
        projector.update(near_start)

        self.assertEqual(projector.published_position_us(), 0)

    def test_paused_session_carries_lead(self) -> None:
        projector = PositionProjector(clock=self.clock, lead_us=400_000)
        paused = copy.deepcopy(self.session)
        paused["playbackState"]["state"] = "paused"
        projector.update(paused)
